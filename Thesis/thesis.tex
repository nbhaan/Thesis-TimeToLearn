...
During the project several sources had been consulted. The different sources that are explained in this section are: Zeeguu, Tizen, comparable researches and the best way to study words.

The smartwatch app is connected to the profile of the user on Zeeguu. Zeeguu is a platform designed for saving foreign words to increase vocabulary of that language. The smartwatch runs on Tizen, the operating system designed by Samsung.
The smartwatch is a relatively new gadget and therefore the number of researches about the smartwatch are limited. There are however comparable researches about a smartphone with the same purpose, that is making learning available wherever the user is. 
Learning new words is the main purpose of the app and thus it is important to research the best way to learn words. Especially when the user will only use micro-learning since the smartwatch app is a complementary tool. 

\section{Zeeguu}
The smartwatch app can only be used when the user has a profile on Zeeguu. The Zeeguu platform was created by Simon Marti (!) to improve learning words of a foreign language. This platform was extended with several new programs to make it more convenient to use for the user. There is a Chrome plugin, several Android applications and iOS applications and after this project a smartwatch app can be added to the list.

The Chrome plugin can be installed to quickly add new words to the users profile. With the plugin the user can click words the user wants to learn. By clicking the translation of that word will appear above it and by clicking the translation the word is saved to the profile of the user. When a translation does not appear to be correct, the user can change the translation. 

\section{Comparable researches}

D. Dearman and K.N. Truong presented a Vocabulary Wallpaper which is a language tool designed to provide quick access to vocabulary to support short session of learning. This app for the smartphone shows some similarity compared to the app for the smartwatch. The Vocabulary Wallpaper should make learning more efficient and easier since the words are shown on the lock screen. Therefore the user does not have to unlock the phone before the app can be used. An interaction with the lock screen is sufficient. This idea is similar to the idea for the smartwatch app. Through a study [...] Dearman and Truong showed that in four short sessions (totaling ~4 hours of exposure), participants were able to increase the number of second language vocabulary that they could recognize and recall when using the Vocabulary Wallpaper.
...
Smartphone is mobile but we need to carry, which implies while we are not carrying it, there could be a chance that we forget where we place the phone. Hence finding or searching for the misplaced phone is another time-taking exercise, compared to wearing a smartwatch.

\section{The best way to study words}

There are a lot of researches about finding the most efficient way for learning new words. Due to the large number of studies, it was not possible to implement all the conclusions of each research. Those who did not make the final version of the app could be implemented in the future.
