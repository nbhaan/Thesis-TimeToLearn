...
At first the idea for the background of the time was that it should support the current time. Therefore four different backgrounds were designed for the morning, afternoon, evening and night (see figure ..). The space for the time then changed to 50\% of the screen and therefore the backgrounds were unusable because of the old dimensions of the images.
Instead of changing the images to the new dimensions this moment was used to think again about the background of the time. The different images for the different types of the day have to be saved on the watch. This storage issue was solved by using one image. The new background consisted of the sun, the moon and a transition between blue and black. This background rotated around it's center which makes it possible to support the current time with the positions of the sun and the moon (see figure ..). Due to the rotation a sunset and sunrise could be seen on the watch. To emphasize the sunset and sunrise a landscape was added to make, for example, the sun appear from the horizon with a sunset (see figure ..). The used colors for the landscape were different from the colors in the rotating background and hence a new background with the sun and moon was designed to match the landscape. The landscape worked really well with the rotating background and therefore two other landscapes were designed from which the user can choose (see figure ..).

\subsection{Buttons}

As mentioned before swipe detection was insufficient so it was decided that the app should only use touch events. To navigate through the app with only touch events it seemed logical to use buttons. With the time on top of the screen and the wordpairs in the middle there was unused space in the bottom for the buttons. In the first design the user should be able to reveal the translation of the shown word or to go to the next wordpair. This resulted in two buttons, a red one with glasses on it and one with an arrow to the right (see figure ..). Glasses were chosen for revealing because it could refer to looking something up and that matches with revealing the translation. Since the app is displayed on a small screen, the buttons should not be to small and thus almost all the available space in the bottom was used for the buttons to ensure that there is enough space for the user to press a button. Different tests showed that the buttons could be made smaller. This was a favorable conclusion since after testing it was decided that only 50\% will be used for the wordpairs (see figure ..). The design of the buttons was not really pleasing and needed some rethinking. The first modification was erasing the reveal button. Instead of the button the user could touch the word to reveal the translation. The 'next' button was widened to fit on the whole screen (see figure ..). 
During development more options were implemented and therefore a new button was made next to the 'next' button. The settings button gave access to these options (see figure ..). The curves on the buttons as can be seen in the first design were replaced with right angles to create a modern feeling.
<<<<<<< HEAD
The app was tested and more attention was paid to improve the order in which the wordpairs are shown. This process should become smarter by use a knowledge estimator that would estimate what the next word should be. This estimation could only be made when the server could receive feedback of every single word. It is therefore necessary that the user should give feedback after every word. The design of the app was adapted to this new idea by changing the 'next' button to a 'reveal' button (see figure ..). When the user pressed this button the translation appeared and the 'reveal' button changed in three different buttons: a 'wrong' button, a 'menu' button and a 'right' button. This new design forced the user to give feedback of every word. This feedback could be used to optimize the order of the words. The first design of the buttons were a cross and a checkmark to indicate the user what to press when a word was not know or was known. Later on a book and a graduation cap were used for the same purpose but the feedback to the user would be less harsh with the new design.

\subsection{Information components}

date: added later, own small icon on top of the screen
temperature and weather type added after the date, first to the left and the right of the date, after a while a new icon was designed for the date, temperature and weather type.
...
popup
=======
The app was tested and more attention was paid to improve the order in which the wordpairs are shown. This process should become smarter by use a knowledge estimator that would estimate what the next word should be. This estimation could only be made when the server could receive feedback of every single word. It is therefore necessary that the user should give feedback after every word. The design of the app was adapted to this new idea by changing the 'next' button to a 'reveal' button (see figure ..). When the user pressed this button the translation appeared and the 'reveal' button changed in three different buttons: a 'wrong' button, a 'menu' button and a 'right' button. This new design forced the user to give feedback of every word. This feedback could be used to optimize the order of the words. The first design of the buttons were a cross and a checkmark to indicate the user what to press when a word was not know or was known. Later on a book and a graduation cap were used for the same purpose but the feedback to the user would be less harsh with this new design.

\subsection{Information components}

Besides the time the app show also the date. The date was added later when the design of the time was completed. In the first place the date had his own small icon on top of the screen (see figure ..). This icon was partly covered by a Tizen icon for the swipe down menu. Therefore the icon for the date was moved downwards near the time.
During a discussion it was mentioned that it would be nice to be able to see the temperature and the weather type on the watchface. After some research a free api was found that could provide the app with this information. In the beginning of the implementation the temperature was placed to the left of the date and the weather type was placed to the right of the date. This was barely readable and it did not fit in the realized design hence a new icon was designed that contained the date, temperature, weather type and an open area in the middle for the Tizen icon for the swipe down menu (see figure ..).
...
When the app was able to show the words with the translation, the user should have an option to tell the app that a word had a wrong translation or that a word was learned and thus it should not reappear again. Due to the small screen it was not convenient to add more buttons to the watchface and therefore the idea came to make a menu with these extra options (see figure ..). Besides these options, the user should be able to reverse the order in which the wordpairs were asked (when the words are displayed from German to English, the words will be displayed from English to German after the button was pressed), to insert the number of words that the user wanted to learn and to log out of the app. All these options were reachable for the user by a 'menu' button in the bottom. The 'next' button was replaced by two buttons, the 'menu' and the 'next' button (see figure ..). When the 'menu' button was clicked a menu would appear with the 'trash' button and the 'I learned it' button.  In the bottom the user could close the app or open the settings (see figure ..). By pressing the 'settings' button the user would open the settings page with the buttons for reverse, number of words and log out (see figure ..).
After some discussions the overall opinion was that the 'menu' button should not be this large on the watchface. A solution was to split the menu and the settings to two different pages that the user could reach independently. The user could reach the menu by tapping in the middle of the translation (see figure ..) and the settings could be reach by double tapping on the time since this space was hardly used. The double tap was necessary because tapping the time would change the background.
...
During testing the text in the buttons were hard to read and the buttons of the main page were visible through the buttons on the bottom. Therefore it was decided that the menu and the settings page should have a background. For the background the same image was used as the background of the wordpairs to preserver the unity of the design. Later on  the idea was abandoned of letting the user choose how many words the user wants to learn and instead came the 'profile' button as the latest addition to the settings page (see figure ..).

\subsection{Profile}

One of the latest addition to the app was the implementation of a profile page where the user could see the four medals that could be earned by using the app (see figure ..). The four categories are: words learned, total time, longest session and longest streak. 
...
When the user pressed the 'I learned it' button, the number of 'words learned' is increased and after 10 new learned words, a popup appears with a motivating message to continue. 
The total time the user used the app is also registered. When this time is the same as one of the pre-determined minutes a popup appears (see figure ..).
With longest session the time is measured that user uses the app  continuously and with the longest streak the number of days is registered in which the app is used daily. One day of not using the app resets the longest streak.
