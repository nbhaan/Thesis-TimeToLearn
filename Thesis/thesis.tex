...
D. Dearman and K.N. Truong presented a Vocabulary Wallpaper which is a language tool designed to provide quick access to vocabulary to support short session of learning \ml{Insert cite command here!}\ml{the same goes for all references from now on. The citation goes at the end of the sentence which introduces an approach}. This app for the smartphone shows some similarity compared to the app for the smartwatch. The Vocabulary Wallpaper should make learning more efficient and easier since the words are shown on the lock screen. Therefore the user does not have to unlock the phone before the app can be used. An interaction with the lock screen is sufficient. This idea is similar to the idea for the smartwatch app. Through a study [...] Dearman and Truong showed that in four short sessions (totaling ~4 hours of exposure), participants were able to increase the number of second language vocabulary that they could recognize and recall when using the Vocabulary Wallpaper.
